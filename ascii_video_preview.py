...
def frame_to_ascii(gray: np.ndarray, params: AsciiParams) -> list[str]:
    ...
    # 0 -> darkest -> last char, 255 -> brightest -> first char (or reverse depending on charset convention)
    # Here: 0 = dark => dense char (end), 255 = bright => space-ish (start)
...
def render_ascii_image(lines: list[str], font: ImageFont.FreeTypeFont, pad: int = 8,
                       fg=(245, 245, 245), bg=(10, 10, 10)) -> Image.Image:
    # Determine text bbox
    # Use a sample "M" to approximate cell size
    ascent, descent = font.getmetrics()
    # Pillow doesn't give precise monospace cell width reliably; use getlength on "M"
    char_length = None
    try:
    ...
    if char_length is not None:
        cell_w = max(1, int(math.ceil(char_length)))
    else:
        cell_w = 1
    cell_h = max(1, ascent + descent)

    cols = max(len(s) for s in lines) if lines else 0
    rows = len(lines)
...
def apply_mask_to_ascii_lines(lines: list[str], mask: np.ndarray | None) -> list[str]:
    ...
    for r in range(rows, len(lines)):
        masked.append(lines[r])
    return masked
...
; Script generated by ascii_video_preview.py
Title: ASCII Export
ScriptType: v4.00+
WrapStyle: 0
...
def escape_ass_text(s: str) -> str:
    # Replace only the sequences that would break ASS parsing, but keep literal
    # characters so the ASCII grid alignment stays intact.
    result: list[str] = []
    trigger_next = {"N", "n", "h", "H"}
    length = len(s)

    idx = 0
    # convert leading spaces to \h so ASS keeps them
    ...
    while idx < length:
        ...
        if ch == "\\":
            ...
            if next_char in trigger_next:
                result.append("\\\\")  # literal backslash before control tokens
            elif next_char == "":
                result.append("\\")
            else:
                result.append(ch)
...
def export_ass(video_path: Path, out_path: Path, params: AsciiParams, start_sec: float,
               dur_sec: float | None, pos_x: int, pos_y: int, fontname: str, fontsize: int,
               play_res_x: int, play_res_y: int,
               mask_lookup: Callable[[int], np.ndarray | None] | None = None) -> None:
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        while True:
            ...
            # Seek by ms (works reasonably; exactness depends on codec)
            ...
            start = sec_to_ass_time(t0)
            end = sec_to_ass_time(t1)

            # \an7 top-left anchored, \pos for placement
            ass_line = f"Dialogue: 0,{start},{end},Default,,0,0,0,,{{\\an7\\pos({pos_x},{pos_y})}}{txt}\n"
            f.write(ass_line)
            i += 1
